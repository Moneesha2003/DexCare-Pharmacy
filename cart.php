...
// Fetch the cart items from the database, grouped by PharmacyID
$sql = "
    SELECT
        c.CartID,
        p.ProductID,
    ...
    WHERE c.UserID = ?
    ORDER BY c.PharmacyID";  // Order by pharmacy to group items
        ...
        <?php if (!empty($cart_by_pharmacy)): ?>
            <?php foreach ($cart_by_pharmacy as $pharmacy_id => $cart): ?>
                ...
                <div class="total-section">
                    ...
                    <a href="checkout.php?pharmacy_id=<?php echo $pharmacy_id; ?>" class="checkout-button">Proceed to Checkout for <?php echo htmlspecialchars($cart['PharmacyName']); ?></a>
                </div>
                <hr>
            <?php endforeach; ?>
        <?php else: ?>
